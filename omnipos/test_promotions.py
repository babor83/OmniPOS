...
1. Partial-paid regression — `omnipos/api/invoices.py:_process_invoice` now
...
import omnipos  # noqa: F401 — ensure app hooks load.
from omnipos.api.invoices import apply_offers, submit_invoice, update_invoice
...
def run_all():
	...
	omnipos.test_promotions.run_all` on a dev site where `bench run-tests`
	would wipe data. In CI, `bench run-tests --app omnipos` picks the same
