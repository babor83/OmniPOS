...
from omnipos.api import credit_sales
...
class TestCreditSales(unittest.TestCase):
	@patch("omnipos.api.credit_sales.frappe.throw", side_effect=_raise_runtime_error)
	@patch("omnipos.api.credit_sales.frappe.qb.from_")
	...
	@patch("omnipos.api.credit_sales._create_credit_allocation_journal_entry")
	@patch("omnipos.api.credit_sales._validate_and_lock_invoice_credit")
	@patch("omnipos.api.credit_sales.frappe.get_doc")
	...
	def test_create_payment_entry_from_advance_rejects_non_customer_receive_entries(self, _mock_throw):
		...
		with patch("omnipos.api.credit_sales.frappe.get_doc", return_value=payment_entry):
