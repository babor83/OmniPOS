	...
	try:
		...
		if coupon_code:
			...
			if frappe.db.table_exists("POS Coupon"):
				from omnipos.omnipos.doctype.pos_coupon.pos_coupon import check_coupon_code
...
def check_offline_invoice_synced(offline_id):
	...
	"""
	from omnipos.omnipos.doctype.offline_invoice_sync.offline_invoice_sync import (
		OfflineInvoiceSync,
	)
...
def submit_invoice(invoice=None, data=None):
	...
	try:
		...
		if coupon_code:
			...
			if frappe.db.table_exists("POS Coupon"):
				try:
					from omnipos.omnipos.doctype.pos_coupon.pos_coupon import increment_coupon_usage
		...
		if redeemed_customer_credit and not invoice_doc.payments:
			invoice_doc.flags.omnipos_redeemed_customer_credit = flt(redeemed_customer_credit)
		...
		if is_credit_sale and not invoice_doc.payments and flt(invoice_doc.grand_total) > 0:
			...
			if not allow_credit_sale:
				frappe.throw(_("Credit sales are not enabled for this POS Profile."))
			invoice_doc.flags.omnipos_credit_sale = 1
		...
		if invoice_doc.get("is_return") and invoice_doc.get("return_against"):
			from omnipos.omnipos.doctype.wallet_transaction.wallet_transaction import (
		...
		if redeemed_customer_credit and customer_credit_dict:
			try:
				from omnipos.api.credit_sales import redeem_customer_credit
