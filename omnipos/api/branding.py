...
def get_branding_config():
	...
	try:
		...
		frappe.log_error(f"Error fetching branding config: {e!s}", "Omnilab Branding API")
		return get_default_config()
...
def validate_branding(client_signature=None, brand_name=None, brand_url=None):
	...
	try:
		...
		frappe.db.set_value("Omnilab Branding", doc.name, "last_validation", frappe.utils.now())
		frappe.db.commit()
		...
		frappe.log_error(f"Error validating branding: {e!s}", "Omnilab Branding Validation")
...
def log_client_event(event_type=None, details=None):
	...
	try:
		...
		if event_type in ["removal", "modification", "hide", "integrity_fail", "visibility_change"]:
			...
			frappe.log_error(
				title="Omnilab Branding - Link Click",
				message=json.dumps(
		...
		frappe.log_error(f"Error logging client event: {e!s}", "Omnilab Branding Event Log")
...
def log_tampering_attempt(doc, details):
	...
	try:
		...
		current_attempts = frappe.db.get_value("Omnilab Branding", doc.name, "tampering_attempts") or 0
		frappe.db.set_value("Omnilab Branding", doc.name, "tampering_attempts", current_attempts + 1)
		frappe.db.commit()

		# Create error log
		frappe.log_error(
			title="Omnilab Branding - Tampering Detected",
			message=json.dumps(details, indent=2, default=str),
		)
	except Exception as e:
		frappe.log_error(f"Error logging tampering: {e!s}", "Omnilab Branding")
...
def get_tampering_stats():
	...
	try:
		...
		frappe.log_error(f"Error getting tampering stats: {e!s}", "Omnilab Branding Stats")
		return {"error": str(e)}
