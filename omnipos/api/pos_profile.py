...
from omnipos.api.utilities import _parse_list_parameter, check_user_company
...
def get_pos_profile_data(pos_profile):
	...
	from omnipos.api.items import get_item_groups
...
def get_pos_settings(pos_profile):
	...
	from omnipos.api.constants import DEFAULT_POS_SETTINGS, POS_SETTINGS_FIELDS
...
def update_pos_profile(*args, **parameters):
	...
	try:
		cache = frappe.cache()
		# Brands cache (used by omnipos.api.items.get_brands)
		cache.delete_value(f"pos_brands:{pos_profile.name}")
		# Item groups cache (used by omnipos.api.items.get_item_groups)
