# Copyright (c) 2025, Omnilab and contributors
# For license information, please see license.txt

"""
Omnilab Branding Monitor
...
def monitor_branding_integrity():
	...
	"""
	try:
		# Check if Omnilab Branding doctype exists
		if not frappe.db.exists("DocType", "Omnilab Branding"):
			return

		doc = frappe.get_single("Omnilab Branding")
		...
		if not doc.encrypted_signature:
			frappe.log_error(
				title="Omnilab Branding - Missing Signature",
				message="Branding configuration is missing encrypted signature. Please resave the Omnilab Branding document.",
			)

		# Log monitoring activity
		frappe.logger().info(
			f"Omnilab Branding Monitor - Checked at {now()}, Tampering attempts: {doc.tampering_attempts or 0}"
		)

	except Exception as e:
		frappe.log_error(
			title="Omnilab Branding Monitor Error", message=f"Error running branding monitor: {e!s}"
		)
...
def send_tampering_alert(doc):
	...
	try:
		...
		# Create notification
		message = f"""
<h3>Omnilab Branding - Excessive Tampering Detected</h3>
<p>The Omnilab branding footer has been tampered with <strong>{doc.tampering_attempts}</strong> times.</p>
...
<p><a href="/app/brainwise-branding">View Branding Configuration</a></p>
				...
				frappe.get_doc(
					{
						...
						"subject": "Omnilab Branding - Tampering Alert",
						"type": "Alert",
						"document_type": "Omnilab Branding",
		...
		frappe.log_error(
			title="Omnilab Branding Alert Error", message=f"Error sending tampering alert: {e!s}"
		)
...
def reset_tampering_counter():
	...
	try:
		...
		if old_count > 0:
			frappe.log_error(
				title="Omnilab Branding - Monthly Counter Reset",
				message=json.dumps(
					{
		...
		# Reset counter
		frappe.db.set_value("Omnilab Branding", doc.name, "tampering_attempts", 0)
		frappe.db.commit()

	except Exception as e:
		frappe.log_error(
			title="Omnilab Branding Counter Reset Error",
...
def validate_all_active_sessions():
	...
	try:
		...
		if active_sessions:
			frappe.logger().info(f"Omnilab Branding - Active sessions: {len(active_sessions)}")

	except Exception as e:
		frappe.log_error(
			title="Omnilab Branding Session Validation Error",
