...
import omnipos  # noqa: F401 — ensure app hooks run (packed_item keying patch).
...
class TestPackedItemsNoDuplicates(FrappeTestCase):
	def test_strip_server_managed_fields_removes_packed_items(self):
		...
		from omnipos.api.invoices import _strip_server_managed_fields
	...
	def setUp(self):
		super().setUp()
		self.assertTrue(
			getattr(packed_item_module, "_omnipos_packed_item_keying_patched", False),
			msg="omnipos packed_item patch must be active (import omnipos before ERPNext saves).",
		)
