...
class OmnilabBranding(Document):
	...
	def validate(self):
		...
		if not self.is_new():
			...
			if protected_fields_changed:
				...
				frappe.log_error(
					title="Omnilab Branding - Fields Modified with Master Key",
					message=json.dumps(
						{
		...
		if not self.enabled and not self.is_new():
			if not self.master_key_provided or not self._validate_master_key():
				frappe.throw(
					...
					"Contact Omnilab support if you need to disable branding.",
					frappe.PermissionError,
				)
	...
	def _validate_master_key(self):
		...
		try:
			...
			if key_hash == MASTER_KEY_HASH and phrase_hash == PROTECTION_PHRASE_HASH:
				...
				frappe.log_error(
					title="Omnilab Branding - Master Key Used",
					message=json.dumps(
						{
							"user": frappe.session.user,
							"timestamp": frappe.utils.now(),
			...
			# Log failed attempt
			frappe.log_error(
				title="Omnilab Branding - Invalid Master Key Attempt",
				message=json.dumps(
					{
						"user": frappe.session.user,
						"timestamp": frappe.utils.now(),
			...
			frappe.log_error(f"Master key validation error: {e!s}", "Omnilab Branding")
			return False
	...
	def validate_signature(self, client_data):
		...
		try:
			...
			frappe.log_error(f"Branding validation error: {e!s}", "Omnilab Branding")
			return False
	...
	def log_tampering(self, details):
		...
		# Create error log
		frappe.log_error(
			title="Omnilab Branding Tampering Detected", message=json.dumps(details, indent=2, default=str)
		)
...
def get_branding_config():
	...
	try:
		...
		frappe.log_error(f"Error fetching branding config: {e!s}", "Omnilab Branding")
		# Return default config even on error
		return {
			"_t": base64.b64encode(b"Powered by").decode(),
			"_l": base64.b64encode(b"Omnilab").decode(),
			"_u": base64.b64encode(b"https://nexus.omnilabes.com").decode(),
			"_i": 10000,
			"_v": True,
			"_e": 1,
		}
...
def validate_branding(client_signature=None, brand_name=None, brand_url=None):
	...
	try:
		...
		frappe.log_error(f"Error validating branding: {e!s}", "Omnilab Branding")
...
def log_client_event(event_type=None, details=None):
	...
	try:
		...
		frappe.log_error(f"Error logging client event: {e!s}", "Omnilab Branding")
...
def verify_master_key(master_key_input):
	...
	try:
		...
		frappe.log_error(
			title=f"Omnilab Branding - Master Key Verification {'Success' if is_valid else 'Failed'}",
			message=json.dumps(
				{
					"user": frappe.session.user,
					"timestamp": frappe.utils.now(),
		...
		frappe.log_error(f"Master key verification error: {e!s}", "Omnilab Branding")
...
def generate_new_master_key():
	...
	# Log this generation
	frappe.log_error(
		title="Omnilab Branding - New Master Key Generated",
		message=json.dumps(
			{
				"user": frappe.session.user,
				"timestamp": frappe.utils.now(),
