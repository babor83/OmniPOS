...
def after_migrate():
	...
	try:
		...
		# doctype sync runs after omnipos's and would overwrite anything we did
...
def reclaim_pos_settings_doctype(quiet=False):
	...
	try:
		frappe.reload_doc("omnipos", "doctype", "pos_settings", force=True)
		frappe.reload_doc("omnipos", "doctype", "pos_barcode_rules", force=True)
		frappe.reload_doc("omnipos", "doctype", "pos_allowed_locale", force=True)
		frappe.db.commit()
	except Exception:
		frappe.log_error(
			title="POS Settings Reclaim Error",
			message="Failed to reload omnipos doctypes\n\n" + frappe.get_traceback(),
		)
		raise
